class CraftManager
{
    ...
    void SetWorkbench(CM_Tools_Werkbank wb)
    {
        m_Workbench = wb;
    }

    CM_Tools_Werkbank GetWB()
    {
        return m_Workbench;
    }
    ...
    void CloseCraft()
    {
        if ( GetGame().GetUIManager().GetMenu() && m_CraftHud)
        {
            ...
            GetWB().RPCSingleParam(CRPc.CLIENT_REQUEST_FREE_WB, new Param1<CM_Tools_Werkbank>(GetWB()), true, null);
            SetWorkbench(null);
        }
    }
