class ActionDeconstructWB: ActionInteractBase
{
	...
	override bool ActionCondition( PlayerBase player, ActionTarget target, ItemBase item )
	{
		if (GetGame().IsClient())
		{
			CM_Tools_Werkbank wb = CM_Tools_Werkbank.Cast(target.GetObject());
	...
	{
		CM_Tools_Werkbank wb = CM_Tools_Werkbank.Cast(action_data.m_Target.GetObject());
		if (wb)
		{
			GetGame().CreateObject("CM_Tools_Werkbank_Kit", action_data.m_Player.GetPosition(), false);
...
{    
	override void OnFinishProgressServer( ActionData action_data )
    {
        ...
        if (action_data.m_MainItem.IsKindOf("CM_Tools_Werkbank_Kit"))
        {
