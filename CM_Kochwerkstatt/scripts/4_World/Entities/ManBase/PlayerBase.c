modded class PlayerBase
{
    ...
    override void OnRPC(PlayerIdentity sender, int rpc_type, ParamsReadContext ctx)
	{
        ...
        if (GetGame().IsClient())
        {
            if (rpc_type == kochwerkstatt.SERVER_SEND_MAIN_DATA)
            {ApplyServerData(ctx);}
            else if (rpc_type == kochwerkstatt.SERVER_SEND_CLOSE_MENU)
            {g_CraftManager.CloseCraft();}
        }
        if (GetGame().IsServer())
        {
            if (rpc_type == kochwerkstatt.CLIENT_REQUEST_CRAFT_ITEMS)
            CraftItems(ctx);
        }
    }
    ...
    override void Init()
    {
		if ( !GetGame().IsServer() || !GetGame().IsMultiplayer() )
		{
			...
			GetDayZPlayerType().AddItemInHandsProfileIK("CM_Grinder", "dz/anims/workspaces/player/player_main/weapons/player_main_2h_extinguisher.asi", twoHanded,		"dz/anims/anm/player/ik/two_handed/BatteryCar.anm");
			
				
			GetDayZPlayerType().AddItemInHandsProfileIK("CM_Kochwerkstatt", "dz/anims/workspaces/player/player_main/player_main_heavy.asi", heavyItemBehaviour,               "dz/anims/anm/player/ik/heavy/55galDrum.anm");
           	
			GetDayZPlayerType().AddItemInHandsProfileIK("CM_Drill", "dz/anims/workspaces/player/player_main/player_main_heavy.asi", heavyItemBehaviour,                      "dz/anims/anm/player/ik/heavy/tent_party_packed.anm");
			
			GetDayZPlayerType().AddItemInHandsProfileIK("CM_Cutting_Saw", "dz/anims/workspaces/player/player_main/player_main_heavy.asi", heavyItemBehaviour,                      "dz/anims/anm/player/ik/heavy/tent_party_packed.anm");
        }
        super.Init();         
    }
}