class CraftHud extends UIScriptedMenu
{
    ...
    private const string RedBtn = "CM_Kochwerkstatt/scripts/4_World/layout/redBtn.edds";
    private const string GrnBtn = "CM_Kochwerkstatt/scripts/4_World/layout/greenBtn.edds";
	...
	{
		layoutRoot 				    = 	GetGame().GetWorkspace().CreateWidgets("CM_Kochwerkstatt/scripts/4_World/layout/Workbench.layout");
        ...
        {
            m_MainImg.LoadImageFile(0, "CM_Kochwerkstatt/scripts/4_World/layout/Workbench3.edds");
        }
        FillRecipes();
        FillPosibleRecipes();
		return layoutRoot;
    ...
    void StartProggres()
    {
        ...
        if (m_ProgressCraftBar.GetCurrent() == 100)
        {
            ...
            if (pl)
            {
                pl.RPCSingleParam(kochwerkstatt.CLIENT_REQUEST_CRAFT_ITEMS, new Param2<CraftItem, ItemBase>(crItm, g_CraftManager.GetWB()), true, null);
            }
            GetGame().GetCallQueue( CALL_CATEGORY_GAMEPLAY ).Remove(StartProggres);
            progressCounter = 0;
            m_ProgressCraftBar.Show(false);
            GetGame().GetCallQueue( CALL_CATEGORY_GAMEPLAY ).CallLater( ReloadBench, 1, false);
        }
    }
